...
ui <- fluidPage(
  ...
  # ABOVE TABS ----
  titlePanel(tags$h3("Impact of COVID-19 pandemic on acute care provision")),
  
  # global selector for health board
  fluidRow(
    #column(width = 6,
    selectInput(inputId = "hb",
                label = tags$body(tags$b("Which health board?"),HTML("<br>"),"Select a health board to focus on"),
                choices = hbs_list,
                selected = "S08000015"),
  ),
  # column(width = 6,
  ...
  tabsetPanel(
    ...
    # commenting out for deployed shiny app
    # 
    ...
    tabPanel(tags$b("COVID impact on bed occupancy"),
             ...
             fluidRow(
               ...
               column(width = 3, offset = 3,
                      "Occupancy of selected health board is presented in purple; data for whole of Scotland is presented in dark blue for comparison."
                      )
             ),
